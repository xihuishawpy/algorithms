        ...
        for i in key_value_string:
            ...
            if _token[0]:
                ...
                if length == 1:
                    if (
                        _token
                        and _token[0] not in dict
                        and (
                            params_to_strip
                            and _token[0] != params_to_strip[0]
                            or not params_to_strip
                        )
                    ):
                    ...
                    if (
                        _token
                        and letter not in dict
                        and (
                            params_to_strip
                            and letter != params_to_strip[0]
                            or not params_to_strip
                        )
                    ):
...
def strip_url_params2(url, param_to_strip=[]):
    ...
    for i in range(len(queries_obj) - 1, 0, -1):
        if (
            queries_obj[i] in param_to_strip
            or queries_obj[i] in queries_obj[:i]
        ):
            queries.pop(i)
